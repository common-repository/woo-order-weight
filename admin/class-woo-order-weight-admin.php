...
class Woo_Order_Weight_Admin {
	 ...
	 */
	 ...
	 public function woo_populate_weight_column( $column, $post_id ) {
     if ( 'order_weight' === $column ) {
         // Use the $post_id to fetch the order
         $order = wc_get_order( $post_id );

         if ( $order && is_a( $order, 'WC_Order' ) ) {
             // Retrieve the stored weight for this order
             $weight = get_post_meta( $order->get_id(), $this->woo_get_weight_meta_key(), true );

             if ( $weight > 0 ) {
                 print esc_html( $weight . ' ' . $this->woo_get_woocommerce_weight_unit() );
             } else {
                 print '<span aria-hidden="true">&#151;</span>'; // Dash if no weight is set
             }
         }
     }
 }
